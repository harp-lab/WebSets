// ============================================================================
// Unified Set Availability - Write Kernel By Key (Sentinel + VT=12)
//
...
// 6. Scatter result keys and values to output using global offset + local offset
//
...
// ============================================================================
// Bindings (11 total for by-key variant)
// ============================================================================
...
@group(0) @binding(5) var<storage, read> offsets: array<u32>;  // Prefix sum of counts
...
// ============================================================================
// Serial Set Operation By Key (ModernGPU Style) - with sentinel offset
//
...
//
// OP_MODE result values (by key):
//   0,1 (intersection, difference): always a_key + a_val
//   2,3 (union, sym_difference):    select(a, b) key+val based on pB
// ============================================================================
...
// ============================================================================
// Main Kernel: Write Set Operation Results By Key
// ============================================================================
@compute @workgroup_size(256)
fn write_availability_by_key(
    ...
    // ========================================================================
    // Step 4: Serial set operation by key (re-execute to get results)
    // ========================================================================
    ...
    let commit = serial_set_operation_by_key(
        ...
        b_start + b0tid,
        b_start + b_count2,
        b_start,         // b_start_base: base of B in shared memory
        b0,              // b_global_offset: global start of B for this workgroup
        star,
        b_adjust,
        extended,
        &result_keys_arr,
    ...
    // ========================================================================
    // Step 6: Scatter result keys and values to output
    // ========================================================================
    ...
    for (var i: u32 = 0u; i < VT; i++) {
        if ((commit & (1u << i)) != 0u) {
            output_keys[write_pos] = result_keys_arr[i];
            output_values[write_pos] = result_values_arr[i];
            write_pos++;
        }
    }
}
