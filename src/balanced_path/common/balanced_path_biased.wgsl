...
fn compute_diagonals(@builtin(global_invocation_id) global_id: vec3<u32>,
    ...
    // Bounded loop with all subgroup ops unconditional to satisfy WGSL
    // uniformity analysis. Uses select() instead of if/break.
    for (var iter: u32 = 0u; iter < 32u; iter++) {
        ...
        // --- All subgroup operations called unconditionally ---
        ...
        // --- End of subgroup operations ---

        // Update result only on first find (branchless)
        let found = first_bit < sg_size;
        fx = select(fx, found_x, found && !done);
        fy = select(fy, found_y, found && !done);
        done = done || found;

        // Shrink search window (branchless, skip when done)
